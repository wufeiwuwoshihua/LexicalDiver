...
is_sat(toured(Wednesday) == Operations) ::: (E)'''
    completion_8 = "# Declarations\nchildren = EnumSort([Fred, Juan, Marc, Paul, Nita, Rachel, Trisha])\nlockers = EnumSort([1, 2, 3, 4, 5])\nassigned = Function([children] -> [lockers])\nshared = Function([lockers] -> [bool])\n\n# Constraints\nForAll([c:children], Exists([l:lockers], assigned(c) == l)) ::: each child must be assigned to exactly one locker\nForAll([l:lockers], Or(Count([c:children], assigned(c) == l) == 1, And(Count([c:children], assigned(c) == l) == 2, Exists([b:children], And(b in [Fred, Juan, Marc, Paul], assigned(b) == l)), Exists([g:children], And(g in [Nita, Rachel, Trisha], assigned(g) == l))))) ::: Each locker must be assigned to either one or two children, and each shared locker must be assigned to one girl and one boy\nExists([c:children], And(c in [Nita, Rachel, Trisha], assigned(c) == assigned(Juan))) ::: Juan must share a locker\nForAll([c:children], Implies(c == Rachel, Not(shared(assigned(c))))) ::: Rachel cannot share a locker\nForAll([l:lockers], Implies(Or(assigned(Nita) == l, assigned(Trisha) == l), Not(Or(assigned(Nita) == l + 1, assigned(Nita) == l - 1, assigned(Trisha) == l + 1, assigned(Trisha) == l - 1)))) ::: Nita's locker cannot be adjacent to Trisha's locker\nassigned(Fred) == 3 ::: Fred must be assigned to locker 3\nAnd(assigned(Juan) == 5, assigned(Fred) == 3, assigned(Marc) == 4, assigned(Paul) == 2) ::: the four boys are assigned to consecutively numbered lockers and Juan is assigned to locker 5\n\n# Options\nQuestion ::: which one of the following is a complete and accurate list of lockers each of which CANNOT be a shared locker?\nis_accurate_list(Not(shared(2))) ::: (A)\nis_accurate_list(Not(shared(4))) ::: (B)\nis_accurate_list(And(Not(shared(1)), Not(shared(2)))) ::: (C)\nis_accurate_list(And(Not(shared(1)), Not(shared(4)))) ::: (D)\nis_accurate_list(And(Not(shared(2)), Not(shared(4)))) ::: (E)"

    z3_program = LSAT_Z3_Program(completion_7, 'AR-LSAT')
