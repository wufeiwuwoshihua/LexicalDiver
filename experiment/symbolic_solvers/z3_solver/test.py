...
# Constraints
# Each child is assigned to exactly one locker:
ForAll([c:children], 
    And(
      Exists([l:lockers], assigned(c, l)),
      ForAll([l1:lockers, l2:lockers], 
        Implies(And(assigned(c, l1), assigned(c, l2)), l1 == l2)
      )
    )
)                                         ::: Each child must be in exactly one locker

# Each locker holds one or two children:
ForAll([l:lockers], 
    Or(
      Count([c:children], assigned(c, l)) == 1,
      Count([c:children], assigned(c, l)) == 2
    )
)                                         ::: Each locker has one or two children

# Shared lockers have one boy and one girl:
ForAll([l:lockers], 
    Implies(
      Count([c:children], assigned(c, l)) == 2,
      Exists([b:boys, g:girls], And(assigned(b, l), assigned(g, l)))
    )
)                                         ::: Any shared locker has one boy and one girl
...
)                                         ::: Rachel does not share

# Nita's locker not adjacent to Trisha's:
Not(Abs(assigned(Nita, l1) * l1 - assigned(Trisha, l2) * l2) == 1) ::: Nitas locker ≠ adjacent to Trishas

# Fred in locker3 and Trisha inlocker3:
assigned(Fred, 3)   ::: Fred is in 3
assigned(Trisha, 3) ::: Trisha is in 3

# Marc alone in locker1:
And(
  assigned(Marc, 1),
  Count([c:children], assigned(c, 1)) == 1
)                                         ::: Marc alone in 1

# Options
Question ::: If Trisha in3 and Marc alone in1, which must be true?
