...
class Pyke_Program:
    ...
    def check_specific_predicate(self, subject_name, predicate_name, engine):
        results = []
        # print(f'facts.{predicate_name}({subject_name}, $label)')
    ...
    def execute_program(self):
        ...
        complied_krb_dir = os.path.join(os.path.dirname(__file__), 'compiled_krb')
        if os.path.exists(complied_krb_dir):
            # print('removing compiled_krb')
            for filename in os.listdir(complied_krb_dir):
                file_path = os.path.join(complied_krb_dir, filename)
                try:
